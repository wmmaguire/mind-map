...
- **Hybrid persistence / source of truth:** **`server/READEME.md`** → **Data consistency (hybrid persistence)** — what lives on disk vs Mongo, and known divergence cases (GitHub **#20**, **#44**). **Saved graphs** are stored in Mongo (`Graph.payload`); legacy **`server/graphs/*.json`** import: **`server/scripts/migrate-graphs-to-mongo.js`**.
- **Library uploads** (`GET/POST /api/files`, `POST /api/upload`) and **persisted graphs** (`/api/graphs/*`) are implemented in **`server/routes/files.js`** and **`server/routes/graphs.js`** (mounted from `server.js`). **Auth** (`/api/auth/*`, JWT cookie) lives in **`server/routes/auth.js`**; see **`server/READEME.md`** §3 and **`docs/github-backlog-issues.md`** (**#63**).
