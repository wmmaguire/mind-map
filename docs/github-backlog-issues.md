...
These are tracked as **separate GitHub issues** (titles may vary slightly):

| Topic | Intent |
|--------|--------|
| **JWT-verified owner for library APIs** | On **`GET/DELETE /api/files/*`**, **`GET /api/graphs/*`**, **`POST /api/upload`**, **`POST /api/graphs/save`**, derive **`userId`** from **`mindmap_auth`** (or verify header matches **`jwt.sub`**) instead of trusting **`X-Mindmap-User-Id`** alone (spoofing risk). |
| **Gate legacy `GET /api/files` without scope** | Unscoped handler still enumerates **`metadata/`**; disable or require admin in production. |
| **Authorize `POST /api/analyze` against file ownership** | Ensure **`sourceFiles`** / content cannot be analyzed from another user’s uploads when only session id is shared. |
