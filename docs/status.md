...
- **Identity + auth + Library shell (GitHub #31 foundation, #33 UI, #63 auth)**
  ...
  - **`GuestIdentityBanner`**: sign-in / register modal, signed-in menu (user settings, sign out); optional dev preview when env allows; **`onOpenUpload`** from **`App`** opens the root **`FileUpload`** modal; leading **Home** / **Visualize** / **Library** rails and **Upload** chip (**#40**); signed-in account trigger uses the same rail styling (replaces sidebar **`LibraryAccountChip`**).
