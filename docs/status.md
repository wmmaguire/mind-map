...
Last updated: 2026-04-08 — **#36** phase 1: client-side graph **History** scrubber in **`LibraryVisualize`** + spike **`docs/graph-time-travel-spike.md`** + **`client/src/utils/graphHistory.js`**. **#62** client polish: Generate modal **Apply**, inline validation, **auto-close** on submit, on-canvas **`graph-edit-mode-chip`** with **progress bar** (indeterminate / per-cycle determinate) + **Stop after this cycle** on the chip; docs in **`client/README.md`**, **`docs/github-backlog-issues.md`**. Still: **#37** **`dryRun`** / budget **Preview** not exposed in the Generate UI (server API unchanged). Earlier same day: **#37** server slice, **#63** auth docs, **#58** follow-ups in **`server/READEME.md`** §2b.
