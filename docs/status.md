...
Last updated: 2026-04-13 — **Community evolution (#68, Apr 2026):** **`POST /api/generate-node`** accepts **`anchorStrategy`**, **`existingGraphLinks`**, optional prune (**`enableDeletions`**, **`deletionsPerCycle`**; **`deleteStrategy`** defaults to **−anchorStrategy**), returns **`deletedNodeIds`**; client **Generate** modal + **`mergeGenerateResult`**. **Library timeline (#86):** smoother scrub transitions in **`LibraryVisualize`**. **Mongo graphs:** duplicate **`nodes.id`** unique index across graphs removed (**`metadata.filename`**-scoped uniqueness). Docs: **`client/README.md`**, **`server/READEME.md`**, **`docs/github-backlog-issues.md`** (#68 note).
