...
Last updated: 2026-04-08 — **#39** slice: read-only **share links** (`POST /api/graphs/:filename/share-read-token`, `GET …?shareToken=`), client **`/visualize?shareGraph=&shareToken=`** + **`GraphVisualization` `readOnly`**; **`server/lib/graphShareRead.js`** tests; docs **`server/READEME.md`**, **`client/README.md`**. **#36** phase 1: in-memory graph history (**`graphHistory.js`**), **`GraphHistoryUiContext`** → **`GuestIdentityBanner`** UI (slider, **Play**/**Pause**), **`goToHistoryIndex`** ordered before dependent hooks (TDZ fix); spike **`docs/graph-time-travel-spike.md`**; backlog follow-ons **#70** + **`docs/github-backlog-issues.md`**. **#62** client polish: Generate modal **Apply**, inline validation, **auto-close** on submit, on-canvas **`graph-edit-mode-chip`** with **progress bar** (indeterminate / per-cycle determinate) + **Stop after this cycle** on the chip; docs in **`client/README.md`**, **`docs/github-backlog-issues.md`**. Still: **#37** **`dryRun`** / budget **Preview** not exposed in the Generate UI (server API unchanged). Earlier same day: **#37** server slice, **#63** auth docs, **#58** follow-ups in **`server/READEME.md`** §2b.
...
- **Core UI flows**
  ...
  - **Library + analyze**: `LibraryVisualize.js` + **`LibrarySidebar`**, **`LibrarySourcesPanel`**, **`LibraryAccountChip`**
    ...
    - **#36 (phase 1):** client-only graph **history** snapshots; banner controls via **`GraphHistoryUiContext`** (see **`docs/graph-time-travel-spike.md`**)
