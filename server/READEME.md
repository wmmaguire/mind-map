...
- `lib/graphInsightsAssess.js`: validates **`POST /api/graph-insights-assess`** body (tone presets (`freud`, `palahniuk`, `murakami`, `thompson`, `delanda`, `custom` — **client UI hardcodes `delanda`** after the Perspective `<select>` was removed in Apr 2026; **`jung`** / **`jones`** are not accepted), optional **`guidingFocus`** interpretive: `all` \| `structure` \| `theme` \| `direction`; network lens: `network_all` \| `flow` \| `resilience` \| `emergence`; **`custom`** with **`customGuidingQuestions`** when `custom`; optional **`reflectionBalance`** numeric **0..100** (default **50**, clamped to `[REFLECTION_BALANCE_MIN, REFLECTION_BALANCE_MAX]`; non-numeric returns **400 `INVALID_REFLECTION_BALANCE`**) — `buildReflectionBalanceDirective(n)` maps the value into five prompt directives (**Strongly reflective** ≤20, **Leaning reflective** ≤40, **Balanced** ≤60, **Leaning discovery** ≤80, **Strongly discovery** >80) injected after the guiding-question block; optional **`assessmentLength`** `low` \| `medium` \| `high` — default **`low`**; notable nodes from client centralities); OpenAI chat completion; interpretive or network-lens prompt with **two-phase reading** instructions—**(A)** grounded factual claims about the map from the payload, **(B)** optional speculative extension when clearly distinguished; guiding questions and system/user copy ask for **anchoring** in named concepts before association; output length and `max_tokens` scale with **`assessmentLength`** (e.g. **550** / **1100** / **2200**); metrics inform reasoning privately—see GitHub **#83** and **`docs/github-backlog-issues.md`** (*Interactive network insights*)
- `routes/files.js`: **`/api/files`**, **`/api/upload`**, **`/api/files/:filename`** (library uploads + metadata)
- `routes/auth.js`: **`/api/auth/register`**, **`/api/auth/login`**, **`/api/auth/logout`**, **`GET /api/auth/me`**, **`PATCH /api/auth/me`**, **`POST /api/auth/forgot-password`**, **`POST /api/auth/reset-password`**, and the Google Sign-In surface (**`POST /api/auth/google/nonce`**, **`POST /api/auth/google`**, **`POST /api/auth/google/link`**, **`POST /api/auth/google/revoke`**). JWT **`mindmap_auth`** httpOnly cookie; password reset uses one-time token, **1h** expiry, hashed at rest. Google ID tokens verified via **`server/lib/googleAuthClient.js`** (thin wrapper around `google-auth-library` with an injectable verifier for tests); users resolved by **`resolveGoogleUserWithStore()`** to one of three outcomes (`existing-google` / `link-required` / `new`). Password rows are never auto-merged onto a Google identity; the client must confirm via the 5-min signed link JWT (**GitHub #63 / #31 / #102**). Walkthrough + env vars in **`docs/auth-google.md`**.
...
**Google Sign-In (GitHub #102):** **`POST /api/auth/google/nonce`** mints a 32-byte nonce and sets the short-lived (5 min) httpOnly **`mindmap_google_nonce`** cookie so the server can cross-check the ID token's `nonce` claim on the subsequent exchange. **`POST /api/auth/google`** (`{ credential, nonce }`) verifies the Google ID token via `OAuth2Client.verifyIdToken` (audience = **`GOOGLE_OAUTH_CLIENT_ID`**, issuer ∈ {`https://accounts.google.com`, `accounts.google.com`}, optional workspace **`GOOGLE_OAUTH_ALLOWED_HD`**), then routes to one of three outcomes: (1) re-issue session cookie for an existing `googleId` match; (2) return **409 `LINK_REQUIRED`** + a 5-min signed link JWT when a password row matches the email (never auto-merged); (3) provision a fresh `User` with `{ provider: 'google', passwordHash: null }`. **`POST /api/auth/google/link`** (`{ linkToken }`) finalizes the link after explicit user confirmation; **`POST /api/auth/google/revoke`** clears session + nonce cookies (client is responsible for calling `google.accounts.id.disableAutoSelect()`). **`POST /api/auth/login`** returns **400 `USE_GOOGLE_SIGN_IN`** for Google-only accounts before bcrypt runs. When **`GOOGLE_OAUTH_CLIENT_ID`** is unset the Google endpoints return **503 `GOOGLE_SIGN_IN_DISABLED`** so the client UI hides cleanly and password flow stays available. See **`docs/auth-google.md`** for console setup, env vars, and troubleshooting.
...
5. Server calls OpenAI Chat Completions (model from **`OPENAI_ANALYZE_MODEL`**, default **`gpt-4o`**) and **expects JSON** with:
   ...
   - `links[]` (relationships: source, target, relationship; server fills `strength` in \([0,1]\) — GitHub **#80**)
...
9. **Link strength (GitHub #80):** after thumbnails, the server ensures every link has numeric **`strength`** in \([0,1]\). v1 is a cheap deterministic heuristic based on relationship text + endpoint labels/descriptions (no extra model calls). Saved graphs preserve `strength` via `POST /api/graphs/save`.
10. On success, server updates `GraphTransform` with `result`, marks it `completed`, writes `UserActivity` **`ANALYZE_COMPLETE`** **`SUCCESS`**, then returns the graph JSON. On failure (OpenAI **429** / **401**, parse errors, etc.), returns a non-2xx JSON body with `details` and `code` (e.g. `OPENAI_QUOTA`, `ANALYSIS_FAILED`), marks the transform `failed`, and writes **`ANALYZE_COMPLETE`** **`FAILURE`** when a transform record exists.
...
1. Client sends `POST /api/generate-node` with **`selectedNodes`** (non-empty array, each with **`id`**) and optional **`numNodes`** (default **3**). Optional **`generationContext`** (string, max **2000** characters after trim) is injected into the **first** and **second** OpenAI prompts as **USER GUIDANCE — TONE, VOICE, AND CONCEPT CHOICES**: it biases **which** new Wikipedia-suitable concepts are chosen as well as **how** descriptions and relationship strings are written (must not override required IDs or connectivity). **Manual single-anchor constraint (optional, manual-mode only):** **`requiredAnchorId`** (must equal one of **`selectedNodes[].id`**) pins every new node to that anchor; other highlighted anchors become optional link targets. Two **mutually exclusive** optional sub-constraints can accompany the anchor — send **at most one**: **`requiredRelationshipLabel`** (trimmed string, max **200** characters) is the phrasing the model must use — or closely paraphrase — on every new-node → required-anchor edge; the server also **force-overrides** the relationship string on those edges back to the user's verbatim label after the step-6 synthesis pass so the pinned phrasing survives the rewrite. **`requiredConceptHint`** (trimmed string, max **200** characters) instead pins *what the new nodes are about* — injected into the manual prompt as a **REQUIRED CONCEPT HINT** bullet so the model chooses Wikipedia-suitable concepts that fit the hint AND connect to the required anchor (facts stay factual; no synthesis override). Outside manual mode (community evolution, branch), all three fields are silently dropped. Validation errors: **`REQUIRED_ANCHOR_NOT_SELECTED`**, **`INVALID_REQUIRED_RELATIONSHIP_LABEL`**, **`REQUIRED_RELATIONSHIP_LABEL_TOO_LONG`**, **`INVALID_REQUIRED_CONCEPT_HINT`**, **`REQUIRED_CONCEPT_HINT_TOO_LONG`**, **`CONFLICTING_REQUIRED_CONSTRAINTS`** (both label and hint sent together). **UI surface (as of Apr 2026):** the client exposes this single-anchor path as a dedicated **`🌳 Extend 🌳`** button in the docked **graph node tooltip** (above **Explode**). Clicking it opens a **pop-up Extend modal** where the user picks **Relationship** vs **Concept** (mutually exclusive), types the constraint text, picks a guidance preset, then submits; the tooltip node is sent as **`requiredAnchorId`** and exactly one of **`requiredRelationshipLabel`** / **`requiredConceptHint`** travels with it. The earlier **Manual AI generate** modal fieldset has been removed; that modal is now strict-only. **Growth budgets (GitHub #37):** **`GENERATE_NODE_MAX_NEW_NODES`** (default **5**) and **`GENERATE_NODE_MAX_SELECTED`** (default **12**) cap request size. **Community evolution-only fields:** **`expansionAlgorithm`**, **`existingGraphNodeIds`**, optional **`existingGraphLinks`** (`{ source, target }` pairs), **`anchorStrategy`** (`random` \| `lowCommunity` \| `highCommunity`), optional **`enableDeletions`**, **`deletionsPerCycle`**, optional **`deleteStrategy`** (same enum; defaults to inverse of **`anchorStrategy`**). Invalid requests return **400** with **`code`** such as **`MISSING_SELECTED_NODES`**, **`TOO_MANY_SELECTED`**, **`NUM_NODES_OVER_CAP`**, **`GENERATION_CONTEXT_TOO_LONG`**, **`RANDOM_GROWTH_MISSING_GRAPH`**, **`RANDOM_GROWTH_PRUNE_TOO_MANY`**, etc.
...
5. Server validates new node labels vs **`existingGraphNodes`**, assigns timestamps, then validates topology. **Manual (strict, default):** **`normalizeManualExpansionLinks`** keeps only edges between a new node and a **highlighted** anchor (drops extra model edges to other nodes), accepts **new→anchor** or **anchor→new**, then checks full connectivity — every new node must link to every highlighted anchor. **Manual (single-anchor mode, when `requiredAnchorId` is set):** the same normalization still runs, but the connectivity check only requires that each new node links to **the required anchor**; edges to the other highlighted anchors are preserved when the model returned them but are optional. When **`requiredRelationshipLabel`** is also set, the verbatim label is reapplied to every new-node → required-anchor edge after step 6 so the synthesis pass cannot overwrite the user's phrasing. **Community evolution (`expansionAlgorithm: "randomizedGrowth"`):** optional **`enableDeletions`** / **`deletionsPerCycle`** run **`pickRandomGrowthDeletes`** (**`server/lib/randomGrowthPrune.js`**) **before** new nodes attach (excludes anchors, new nodes, degree-1-only nodes, and nodes that would isolate neighbors). **`deleteStrategy`** (low / random / high **community** bias) defaults to the **inverse** of **`anchorStrategy`** when omitted (so growth and prune lean opposite directions unless the client overrides). **`buildRandomExpansionLinks`** uses **`existingGraphNodeIds`**, optional **`existingGraphLinks`** (for **clustering coefficient** when **`anchorStrategy`** is **`lowCommunity`** or **`highCommunity`**), and **`anchorStrategy`** (**`random`** = uniform degree-weighted pool; **low**/**high** = bias by local clustering). Random edges use templated **`relationship`** strings (“uniform” vs “weighted … clustering”). Successful responses may include **`deletedNodeIds`** when pruning ran. On validation failure, responds with **200** and `{ success: false, error, details }` where applicable.
...
**Goal:** grow along an ordered **path** on the graph (user-highlighted branch): each iteration extends from the current **frontier** (path tip) with **`nodesPerIteration`** new nodes, conditioning on the last **`memoryK`** path nodes and their inter-links. **Topology is server-enforced:** new nodes attach **only to the current frontier** (model-proposed links are ignored) so growth stays on the branch tip. The server runs **all iterations in one request** (multiple OpenAI calls internally). Budgets: **`iterations`** capped by **`GENERATE_BRANCH_MAX_ITERATIONS`** (default **10**, max **20**), **`iterations × nodesPerIteration`** capped by **`GENERATE_BRANCH_MAX_TOTAL_NEW_NODES`** (default at most **40**), **`nodesPerIteration`** ≤ **`GENERATE_NODE_MAX_NEW_NODES`**, **`memoryK`** ≤ **40**.

**Body:** **`existingGraphNodes`**, **`existingGraphLinks`** (`{ source, target }` string ids), **`branch.pathNodeIds`** (length ≥ 2; each consecutive pair must appear as an edge in **`existingGraphLinks`**), **`iterations`**, **`memoryK`** (prompt context only — does not add edges), **`nodesPerIteration`**, **`crossLinksPerIteration`** (deterministic back-edges from new nodes into non-frontier memory-window nodes; default **0**, capped by **`maxCrossLinksPerIteration`** = **6**), optional **`generationContext`**, optional **`dryRun: true`** (returns **`preview`** only). Client UI exposes both **Memory window** and **Cross-links per iteration** sliders under *Extrapolate branch*.
...
**Client:** primary control is the **`💥 Explode 💥`** button on the **selected-node tooltip** (`GraphVisualization`). Clicking it opens a **pop-up Explode modal** that hosts the **Guidance** preset + optional custom text and the **Concepts to add** slider (2–6, default 4); submitting the modal fires `POST /api/explode-node`. There is no duplicate control in the graph Actions AI Generation section.
