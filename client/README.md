...
- `client/src/components/GraphVisualization.js`
  ...
  - **#81 cluster chips:** when zoomed out into merged community view, each cluster renders one small “thumbnail chip” near its centroid. The chip is anchored to the **most-connected node** in that cluster (highest within-cluster degree): it shows the anchor’s thumbnail (when present) + label and is clickable to focus/zoom to that anchor.
