...
- `client/src/components/GraphVisualization.js`
  ...
  - **#83 Insights (`issue-83-graph-insights`):** **View → Insights** — **Network snapshot** (global metrics). **Top by degree** is **collapsible** (collapsed by default). **Assess graph** calls **`POST /api/graph-insights-assess`** with tone + **`buildGraphInsightAssessPayload`** (notable nodes across centralities; server prompt is **psychoanalytically framed** — three guiding questions: symptomatic structure, theme/meaning, projected direction/inspiration — **2–3 short paragraphs**, metrics inform reasoning privately). **Interpretive assessment** block: **Copy** / **Save** (`.txt`, filename from **`GraphTitleContext`** when available) / **Close**. **Focus** on a row uses the same zoom/focus path as **Focus next** (**#94**). Metrics **memoized** while the panel is open. Follow-on polish: **`docs/github-backlog-issues.md`**, **#96** / **#97**.
...
10. **Insights (#83):** **View → Insights** — snapshot metrics; expand **Top by degree**; **Focus** matches **Focus next**. **Assess graph** (API running): pick a **Voice** (includes **Alex Jones** parody preset); run assessment — expect a short **psychoanalytic-style** read (**2–3 paragraphs**); use **Copy** / **Save** / **Close**. Reload confirms **`mindmap.chrome.insightsPanelVisible`**. Tests: **`graphInsights.test.js`**.
