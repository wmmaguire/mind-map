...
- `client/src/components/GraphPlaybackBanner.jsx`
  - Second shell strip (**`App.js`**, below **`GuestIdentityBanner`**): **`GraphHistoryBannerControls`** when history (≥2 steps) or **save** payload is set (**#36**). Styles: **`GuestIdentityBanner.css`** (playback strip classes). Share control: **View** menu → **Share link** (**`GuestIdentityBanner`**, **#39**, **#40**).
...
- Use **AI Generation** (Actions → open the generate form) to request AI expansion (**#37** server budgets / **`dryRun`**, **#62** expansion modes):
  ...
  - In the Generate modal, **Expansion algorithm** selects **Manual** (default), **Community evolution**, or **extrapolate branch** (dropdown); **Actions → Extrapolate branch…** opens the same form with branch mode selected (**#82**).
  ...
  - **Extrapolate branch (#82):** highlight **at least two nodes in click order** along existing edges (path from root toward tip). One **`POST /api/generate-branch`** runs all **iterations** on the server (indeterminate chip until done). Parameters: **nodes per iteration**, **memory window** (**memoryK** path nodes), **cross-links per iteration**. Response is merged with **`mergeGenerateNodeResponse`** like other generate flows. Not available in read-only share mode (no Actions menu).
