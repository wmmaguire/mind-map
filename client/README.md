...
- `client/src/components/FileUpload.js`
  - Upload modal: **Text file** (`.txt` / `.md`) → **`POST /api/upload`**; **Audio → transcript** tab with **Upload file** or in-browser **Record** (`MediaRecorder`), **`POST /api/transcribe`** (Whisper), client-side **25 MB** check, editable transcript, then **`.txt`** via **`POST /api/upload`** (**GitHub #34** / **#35**). Helpers: **`utils/audioRecording.js`**. Tests: **`FileUpload.test.jsx`**, **`audioRecording.test.js`**. The backend allows **multiple files per session** (see **`server/routes/files.js`** / **`server/models/file.js`**).
