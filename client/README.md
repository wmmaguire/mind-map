...
- `client/src/context/GraphChromeUiContext.jsx`
  - **`GraphChromeUiProvider` / `useGraphChromeUi()`** — **GitHub #38:** visibility of the **playback** strip and **graph search** bar on **`/visualize`**, persisted in **`localStorage`** (`mindmap.chrome.playbackStripVisible`, `mindmap.chrome.graphSearchBarVisible`, defaults **true**). Toggled from **`GuestIdentityBanner`** → **View** (**Playback strip**, **Graph search**). **`GraphPlaybackBanner`** respects **`playbackStripVisible`**; **`GraphVisualization`** uses **`graphSearchBarVisible`**. Wrapped in **`client/src/index.js`**. Outside provider (tests), hooks return safe no-ops. Epic follow-ons: **#73**.
- `client/src/utils/graphDiscovery.js`
  - **GitHub #38:** **`normalizeGraphLabel`**, **`nodesMatchingLabelQuery`** (substring + case-fold on **`label`** only), **`createFocusZoomTransform`** for **Focus next** / centering in the viewport. Tests: **`graphDiscovery.test.js`**. Semantic / server search: **#73**.
...
- `client/src/components/GraphVisualization.js`
  ...
  - **#38 Discovery & navigation:** in-canvas **search** (label keyword match), **Focus next** / match count, **Show all** (reset clusters + fit), **minimap** synced to zoom (**rAF**-throttled), docked tooltip. Chrome toggles via **`GraphChromeUiContext`**. Phase **2+** backlog: **#73** (semantic search, N-hop focus, saved lenses, provenance, **interactive** minimap, scale, D3 hardening, read-only policy).
...
9. **Discovery & graph chrome (#38; phase 2 = #73):** With a graph open on **`/visualize`**, use **Search** (substring on **labels**), **Focus next**, and **Show all** (reset merge/split + fit). Confirm the **minimap** tracks the main viewport. In **`GuestIdentityBanner`** → **View**, toggle **Graph search** and **Playback strip**; reload the tab and confirm visibility persists (**`localStorage`**: `mindmap.chrome.graphSearchBarVisible`, `mindmap.chrome.playbackStripVisible`). Automated coverage for toggles: backlog **#24**; semantic search / interactive minimap: **#73**.
