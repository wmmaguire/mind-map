...
- `client/src/context/GraphHistoryUiContext.jsx`
  - **`GraphHistoryUiProvider` / `useGraphHistoryUi()`** — registers **`/visualize`** payloads: **`setPayload`** (history scrubber API), **`setSharePayload`** (read-only link, **#39**), **`setSavePayload`** (**Save** current graph). **`GuestIdentityBanner`** renders **share** left of **View** when `sharePayload` is set; **`GraphPlaybackBanner`** renders save + playback when history or save payloads are set (**#36**). Default no-op when provider absent (tests).
- `client/src/components/GraphPlaybackBanner.jsx`
  - Second shell strip (**`App.js`**, below **`GuestIdentityBanner`**): **`GraphHistoryBannerControls`** when history (≥2 steps) or **save** payload is set (**#36**). Styles: **`GuestIdentityBanner.css`** (playback strip classes). Share control is on the title row (**`GuestIdentityBanner`**, **#39**).
