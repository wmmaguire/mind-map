...
- `client/src/utils/graphInsights.js`
  - **GitHub #83:** **`computeGraphInsights`**, **`graphInsightNodeId`**, **`degreeByNodeId`** on the snapshot; **`computeInsightNotableCentralities`** (Brandes **betweenness**, **closeness**, **eigenvector** on a simple graph; multigraph **degree**); **`buildGraphInsightAssessPayload`** for **`POST /api/graph-insights-assess`**; **`INSIGHT_ASSESS_TONE_OPTIONS`** (ids match server **`TONE_IDS`**: **`freud`**, **`palahniuk`**, **`murakami`**, **`thompson`**, **`custom`** — **Freud** is the default selection in **`GraphVisualization`**; **Palahniuk** replaced former **Jung**); **`INSIGHT_ASSESS_GUIDING_FOCUS_OPTIONS`** (`guidingFocus` + optional **`customGuidingQuestions`**); **`INSIGHT_GUIDING_QUESTION_BODY`** / **`INSIGHT_NETWORK_GUIDING_QUESTION_BODY`** (keep aligned with server **`GUIDING_QUESTION_TEXT`** / **`NETWORK_GUIDING_QUESTION_TEXT`**); **`INSIGHT_CENTRALITY_METRICS_HELP`** + **`formatInsightCentralityScore`**. Tests: **`graphInsights.test.js`**. Advanced diameter/patterns/playback trends/workers: **#83**, **#95**; LLM assess polish (limits, audit, persistence, clipboard a11y): **#96**; client/server tone allowlist parity: **#97**.
- `client/src/context/GraphTitleContext.jsx`
...
10. **Insights (#83):** **View → Insights** — snapshot metrics; expand **Notable by centrality** (four metric lists; **?** per metric for help); **Focus** matches **Focus next**. **Assess graph** (API running): pick **Question** / **Voice** / **length**; run assessment — expect a reading that **grounds** claims about the map in named concepts, then may **extend** with clearly marked inference (length varies with **Low** / **Medium** / **High**); use **Copy** / **Save** / **Close**. Reload confirms **`mindmap.chrome.insightsPanelVisible`**. Tests: **`graphInsights.test.js`**.
