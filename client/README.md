...
- `client/src/components/GraphVisualization.js`
  - D3 force graph rendering + interaction model (select, zoom, pan, edit, delete, generate). **Graph actions** (GitHub **#27**): the old sidebar toolbar was replaced by a floating **Actions** button (top-right of the graph area), a **context menu** on **right-click** on the graph, and a **Graph actions** panel (header, hint, **×** close) for Generate / Add Node / Add Relationship / Delete. Selection for those actions is **snapshotted when the menu opens**. **Add Node** can prompt for **one relationship per highlighted node** when nodes were selected before opening the menu. There is **no long-press on the canvas** to open the menu (avoids fighting pan/zoom and node selection); use **Actions** or **right-click**. The Actions FAB uses **`z-index: 1190`** so it stays **below** the mobile full-screen library sidebar (**`1200`**) and does not cover the library **Close** control. **Styles:** **`GraphVisualization.css`** — obsolete **controls-panel** / **edit-controls** / legacy mobile shell CSS removed in **#28** (lint + Jest clean). Unit tests: **`client/src/components/GraphVisualization.test.js`**. Follow-ups: **`docs/github-backlog-issues.md`** (**#52** z-index audit, **#24** browser E2E, **#51** D3 `useEffect` deps, **#55** optional **`.library-graph-mount`** cleanup).
...
6. **Graph actions (Library → visualize a graph):** Confirm you can **pan/zoom** the graph (drag empty space, wheel or pinch). **Tap/click nodes and edges** to change selection. Open **☰ Actions** (top-right): the menu should appear with **Graph actions** and **×**. Try **Generate**, **Add Node**, **Add Relationship** (with two nodes highlighted), **Delete**; dismiss with **×**, **Actions** again, **outside tap**, or **Escape**. On **desktop**, **right-click** the graph should open the same menu. On **narrow screens**, open the **Library** overlay: the library header **Close** should sit **above** the **Actions** FAB (FAB must not cover **×**).
