...
- `client/src/App.js`
  - Top-level router and page composition. **`FileUpload`** modal and upload-success toast at app root; **`GuestIdentityBanner`** receives **`onOpenUpload={openUploadModal}`** so the shell **Upload** chip opens the same modal on any route (**#40**). **`fileRefreshToken`** is passed to **`LibraryVisualize`** on **`/visualize`** so **Add new** in the library refetches the file list after upload. Landing **`/`** no longer renders the old feature-card grid; primary navigation uses the banner rails (**Home** / **Visualize**) instead.
...
4. **`GuestIdentityBanner`** — shell strip: graph title (**`GraphTitleContext`**), **Sign in / Create account** (or dev **Preview** when enabled), signed-in menu (**User settings** → **`PATCH /api/auth/me`**, **Sign out**), **Home** / **Visualize** / **Library** / **Upload** rails (**#40**), **View** menu (graph search, **Share link** when owner, playback toggle when applicable).
