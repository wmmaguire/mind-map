...
- `client/src/components/LandingPage.jsx` (**#76**)
  - Onboarding page for **`/`** — restores a hero + value-prop cards on top of the **#40** shell (which moved Home / Visualize rails into **`GuestIdentityBanner`**). Sections: **Hero** (eyebrow *Mindmap*, H1 *See the shape of your thinking.*, tagline from **`ValueProposition.md`**, primary **Get Started** CTA with guest-mode hint); **How it works** (three numbered steps: add sources → analyze → explore, preserved from the previous landing); **What you can do** (six feature cards with inline-SVG icons covering ingest, analyze + merge, explore canvas, edit/generate/explode, insights + centrality, two-phase Assess); **Why it matters** (reflective blurb + italicized self-questions evoking `ValueProposition.md`); bottom **Get Started** CTA. Both CTAs use **`useNavigate('/visualize')`** and the existing **`.landing-cta-primary--dynamic`** pulse (**`prefers-reduced-motion: reduce`** disables it). Navigation intentionally stays in **`GuestIdentityBanner`** — no Home/Visualize rails here. Styles: **`LandingPage.css`** (hero, `.landing-feature-grid`, `.landing-feature-card`, `.landing-reflection`, responsive `min-width: 640px` / `960px` breakpoints), extending the existing **`.landing-*`** tokens in **`App.css`**. Tests: **`LandingPage.test.jsx`** (hero + sections render; both CTAs navigate to **`/visualize`**). Follow-ups: **`docs/github-backlog-issues.md`** (landing + guest shell note), GitHub **#76** (optional hero/illustration backlog), **#24** (E2E).
