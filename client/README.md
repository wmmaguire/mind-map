...
**Audio → transcript (same modal):** User switches to **Audio → transcript**, selects a **pre-recorded** audio file (there is **no in-browser microphone recording** yet — **#35**). **`POST /api/transcribe`** with `audio` + `sessionId` returns `{ transcript, model }` (plain text, **no timestamps**). The user can edit the text, then **`POST /api/upload`** saves it as `.txt` so Library **Analyze** works unchanged.
...
4. Optional (**#34**): In **Upload**, open the **Audio → transcript** tab, choose a short audio file (e.g. `.webm` / `.m4a`), **Transcribe** — expect **`POST /api/transcribe`** **200** and an editable transcript — then **Upload transcript as .txt** and confirm **`POST /api/upload`** for the generated text file.
